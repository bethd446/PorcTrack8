...
import {
  ...
  TrendingUp, Scale, AlertTriangle
} from 'lucide-react';
...
const CROISS_PHASE_TONE = 'var(--color-accent-500)';
const CROISS_PHASE_DAYS = FARM_CONFIG.CROISSANCE_DUREE_JOURS ?? 37;
const CROISS_PHASE_OFFSET =
  (FARM_CONFIG.SEVRAGE_AGE_JOURS ?? 28) + (FARM_CONFIG.POST_SEVRAGE_DUREE_JOURS ?? 35);
  ...
  const { portees, summary, occupation } = useMemo(() => {
    ...
    const rows: CroissanceRowData[] = inPhase.map((b) => {
      ...
      const dayInPhase = ageJours !== null
        ? Math.max(0, ageJours - CROISS_PHASE_OFFSET)
        : null;
      const treatment = classifyCyclePhaseCard(
        {
          statut: b.statut,
          dayInPhase,
          phaseDays: CROISS_PHASE_DAYS,
        },
        today,
        'croiss',
      );
    ...
    rows.sort((a, b) => {
      ...
      if (r !== 0) return r;
      const remA = computeRemaining({ dayInPhase: a.dayInPhase, phaseDays: CROISS_PHASE_DAYS }) ?? Number.MAX_SAFE_INTEGER;
      const remB = computeRemaining({ dayInPhase: b.dayInPhase, phaseDays: CROISS_PHASE_DAYS }) ?? Number.MAX_SAFE_INTEGER;
...
const CroissanceCard: React.FC<{ data: CroissanceRowData; onOpen: () => void }> = ({ data, onOpen }) => {
  ...
  const treatmentStyle = getCycleTreatmentStyle(data.treatment, CROISS_PHASE_TONE);
  const isUrgent = data.treatment === 'urgent';
  const isResolu = data.treatment === 'resolu';
  const remainingDays = computeRemaining({ dayInPhase: data.dayInPhase, phaseDays: CROISS_PHASE_DAYS });
