...
function App() {
  ...
  const [detail, setDetail] = useState(null); // { type:'truie'|'bande'|'finances', id }
  const [fabOpen, setFabOpen] = useState(false);
  const [sheet, setSheet] = useState(null); // 'truie' | 'misebas' | 'pesee' | 'reorder' | 'vente' | 'depense' | null
  const [reorderItem, setReorderItem] = useState(null); // { name, qty } for reorder sheet
