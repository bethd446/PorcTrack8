...
- Name: nav-complete.spec.ts >> HubTiles — walk toutes les tuiles >> Hub Cycles — chaque tuile navigue correctement
- Location: tests/e2e/nav-complete.spec.ts:283:5

# Error details

```
Error: Tile "Reproduction" missing in Cycles

expect(locator).toBeVisible() failed

Locator: locator('.ion-page:not(.ion-page-hidden)').last().getByRole('button', { name: 'Reproduction', exact: true }).first()
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Tile "Reproduction" missing in Cycles with timeout 5000ms
  - waiting for locator('.ion-page:not(.ion-page-hidden)').last().getByRole('button', { name: 'Reproduction', exact: true }).first()
