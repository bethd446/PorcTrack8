...
### 4.2 Typographie (4 fonts strict — DNA non négociable)
| Police | Usage | Classe V70 |
|---|---|---|
| **Big Shoulders Display** Bold/900 | Titres H1/H2/labels nav | `--pt-font-display` |
| **Instrument Sans** | Corps de texte | `--pt-font-body` |
| **JetBrains Mono** | Codes IDs / chiffres tabular | `--pt-font-mono` |
| **DMMono** (V77.1) | Métadonnées + sub-text mono | `--pt-font-mono` (fallback) |
...
**État actuel** : tsc=0, 2014/2014 tests, branche `main` à jour, 14 fichiers source touchés cette session.
...
/reproduction       REPRO          ReproV70.tsx        — agenda/en cours/à venir/historique
/performance        PERFORMANCE    PerformanceV70.tsx  — vue/kpis/finances/prévisions
...
### 8.3 🔴 Engraissement — zone faible (audit P0 #2)
- Pas de page dédiée `/lots` ou `LOTS EN CROISSANCE`
- Pesée bande existe via formulaire mais pas de GMQ/IC calculés
- **Critique manquant** : réception lot, pesées hebdo, GMQ/IC auto, alerte poids vente, mortalité par cause, coût total
...
1. **Onboarding profil** (3 boutons Naisseur / Engraisseur / Cycle complet) + adaptation bottom-nav + KPIs + FAB par profil
   - Phase 1a fondation DB + helper (2-3h) — `.claude/PLAN_PROFIL_MULTI.md` §6
   - Phase 1b bottom-nav + FAB (2-3h, agent dédié)
   - Phase 1c KPIs Performance (2-3h, agent parallèle)
2. **Page Engraissement** (lots, pesées, GMQ, IC, coût lot, alerte poids vente) (~8-12h)
3. **Calendrier vaccinal auto** + rappels J-1 + décrément stock (~4-6h)
