          ...
          <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/40 backdrop-blur-sm p-4">
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="bg-white w-full max-w-md rounded-t-[32px] p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <button onClick={() => setShowAdd(false)} className="flex-1 py-4 bg-gray-100 rounded-2xl font-bold text-sm">Annuler</button>
                <button onClick={handleSubmit} className="flex-1 py-4 bg-emerald-600 text-white rounded-2xl font-bold text-sm">Créer</button>
              </div>
            </motion.div>
          </div>
        )}
