...
- Name: nav-complete.spec.ts >> HubTiles — walk toutes les tuiles >> Hub Troupeau — chaque tuile navigue correctement
- Location: tests/e2e/nav-complete.spec.ts:283:5

# Error details

```
Error: Tile "Truies" missing in Troupeau

expect(locator).toBeVisible() failed

Locator: locator('.ion-page:not(.ion-page-hidden)').last().getByRole('button', { name: 'Truies', exact: true }).first()
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Tile "Truies" missing in Troupeau with timeout 5000ms
  - waiting for locator('.ion-page:not(.ion-page-hidden)').last().getByRole('button', { name: 'Truies', exact: true }).first()
